...
def load_cached_data(symbol):
    ...
    if os.path.exists(cache_file):
        ...
        if df.index.tzinfo is None:
            df.index = pd.DatetimeIndex(df.index).tz_localize('UTC').tz_convert(NY_TZ)
        elif df.index.tzinfo != NY_TZ:
...
if __name__ == '__main__':
    # End at current time
    end_date = datetime.now(NY_TZ)  # Current time in ET
    # Start 30 days ago at market open
    start_date = (end_date - timedelta(days=60)).replace(hour=9, minute=30, second=0, microsecond=0)
