...
# Optimized configuration for highest returns with good Sharpe
strategy_configuration = {
    'MFI Period': 9,              # Optimized for returns
    'VWAP Window': 90,            # Any window 90-120 works well
    'ATR Period': 3,              # Fixed
    ...
    'regime_params': {
        'bear_high_vol': {
            'position_scale': 1.75,    # Increased for higher returns
            'reward_risk': 2.5,        # Increased for higher returns
            'stop_mult': 1.2           # Fixed
        },
        'bull_high_vol': {
            'position_scale': 0.5,     # Optimized
            'reward_risk': 2.0,        # Original value
            'stop_mult': 1.2
        },
        'bear_med_vol': {
            'position_scale': 0.3,     # Fixed
            'reward_risk': 2.0,
            'stop_mult': 1.2
        },
        'bull_med_vol': {
        ...
        'bull_low_vol': {
            'position_scale': 0.0,     # Skip low vol regimes
            'reward_risk': 1.5,
            'stop_mult': 1.2
        },
        'bear_low_vol': {
...
class VWAPOBVCrossover:
    def __init__(self, config: Dict):
        ...
        self.vwap_window = config.get('VWAP Window', 90)
        ...
        self.min_stop_dollars = config.get('Min Stop Dollars', 0.50)
        self.max_stop_dollars = config.get('Max Stop Dollars', 2.00)
        self.risk_per_trade = config.get('Risk Per Trade', 0.01)
        self.max_hold_hours = config.get('Max Hold Hours', 24)
        
        # Regime parameters
        self.regime_params = config.get('regime_params', strategy_configuration['regime_params'])
    ...
    def get_regime_parameters(self, regime: str) -> Dict:
        ...
        return self.regime_params.get(regime, self.regime_params['bear_high_vol'])
